...
namespace arrow {
    ...
    DrawFunctionExpressionEvaluator::DrawFunctionExpressionEvaluator(DrawFunctionExpression expression)
      : m_expression(std::move(expression))
    {
    }
    Type DrawFunctionExpressionEvaluator::evaluate(Environment & environment) const
    {
        ...
        auto const r = expressionCollEval[2];
        auto const g = expressionCollEval[3];
        auto const b = expressionCollEval[4];
        ...
        GraphicsCanvas::draw(castedX, castedY, castedR, castedG, castedB);
